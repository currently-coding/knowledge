Let $M = (\sum, S, f, s_0, s_1) \text{ be a TM and }\sum_1 \subseteq \sum \setminus  \{[], *\}$.
A function $\varphi: (\sum_1^*)^n  \rightarrow \sum_1^*$ with $n \geq 0$ is **computed by M** if for all $x_1, ..., x_n \in \sum_1^*$ holds:
?
$$
f(x) = 
\begin{cases} 
