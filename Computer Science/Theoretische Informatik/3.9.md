## Idee
- beim Einlesen von $w$ merkt sich der $DFA$, welches maximale Präfix von $v$ am Ende des gelesenen Wortes steht.
...
### Eigenschaft 2 
- Für alle Wörter $w \in \sum^*$ gilt $\delta' quer(0, w) = P_v(w)
#### Beweis
##### BC 
- $w = \epsilon: \delta ' quer(0, \epsilon) = 0 = P_v(\epsilon)
##### IH
- Für ein $w' \in \sum^*$ gilt 4\delta'(0, w')=P_v(w')$
##### IS
...
\end{cases}
$$
$\text{Damit gilt }(v, w) \in PR \rlarrow \exists x, y \in \sum^* \text{ mit } w = xy \text{ und}$
$(Eigenschaft 2) \exists x, y \in \sum^* \text{ mit }w=xy\text{ und }\delta' quer(0, x) = k ::: w \in L(A_v) \text{, weil Zustand k Senke ist.}$

## Suchautomat $A_v$ für $\sum = \{a, b\} und v=babab$
```Automat
-> s_0: a->s-0;b->s_1;
s_1: a-> s_2; b-> s_1;
s_2: a-> s_0; b-> s_3;
