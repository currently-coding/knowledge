...
var ArrowsSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Default arrow color").setDesc("The default arrow color, in any valid CSS notation. Use `var(--text-normal)` to default to black/white in light/dark mode.").addText((text) => text.setPlaceholder("var(--text-normal)").setValue(this.plugin.settings.defaultArrowColor).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Diagonal arrow style").setDesc("The style of diagonal arrows.").addDropdown((text) => text.addOptions({ straight: "straight", fluid: "fluid", arc: "arc", magnet: "magnet" }).setValue(this.plugin.settings.diagonalArrowStyle).onChange(async (value) => {
      this.plugin.settings.diagonalArrowStyle = value;
      this.plugin.reconfigureArrowsConfig();
      this.plugin.reloadArrowsViewPlugin();
      await this.plugin.saveSettings();
    }));
    const customColorsSetting = new import_obsidian.Setting(containerEl).setName("User-defined colors").setDesc("A list of custom user-defined colors. Arrows whose color is set to a value in this list will have their color overridden by the specified value. Use the format custom-color-name: color, with each color separated by a new line. e.g. my-custom-color: #26edab").addTextArea((text) => text.setValue(this.plugin.settings.userDefinedColors).setPlaceholder("my-custom-color: #26edab\nblue: rgb(0, 98, 255)\ngreen: limegreen").onChange(async (value) => {
...
// src/arrowsViewPlugin.ts
var import_state3 = require("@codemirror/state");
var import_view2 = require("@codemirror/view");

// src/matchDecoratorAll.ts
...
var MatchDecoratorAll = class {
  ...
  createDeco(view) {
    let build = new import_state.RangeSetBuilder(), add = build.add.bind(build);
...
var LeaderLine = function() {
  ...
  function Ie(e2, t2) {
    ...
    if (e2.compareDocumentPosition(i2) & Node.DOCUMENT_POSITION_DISCONNECTED)
      throw "A disconnected element was passed.";
...
var LeaderLine_default = LeaderLine;

// node_modules/leaderline/src/index.js
var _element, _onResize;
var LeaderLine2 = class extends LeaderLine_default {
  constructor(props) {
    props.hide = props.hidden;
    super(props);
    __privateAdd(this, _element, void 0);
    __privateAdd(this, _onResize, void 0);
    __privateSet(this, _element, document.body.querySelector(`:scope>.leader-line:last-of-type`));
    const { parent } = props;
  ...
  remove() {
    ...
    window.removeEventListener("resize", __privateGet(this, _onResize));
    super.remove();
  }
  static PointAnchor(element, options) {
    return LeaderLine_default.pointAnchor(element, options);
  }
  static AreaAnchor(element, options) {
    return LeaderLine_default.areaAnchor(element, options);
  }
  static MouseHoverAnchor(element, options) {
    if (options) {
      options.animOptions = toLeaderLineAnimationOptions(options.animation);
    }
    return LeaderLine_default.mouseHoverAnchor(element, options);
  }
  static Label(text, options) {
    ...
    if (options) {
      ...
      if (!options.outlineColor) {
        options.outlineColor = "transparent";
      }
      ;
    }
    const LeaderLineEntity = LeaderLine_default[on === "path" ? "pathLabel" : "captionLabel"];
    return LeaderLineEntity(text, options);
  }
};
_element = new WeakMap();
_onResize = new WeakMap();
var toLeaderLineDash = (dash) => {
  if (dash && typeof dash === "object") {
    toLeaderLineAnimationOptions(dash.animation);
    dash.len = dash.length;
  }
};
var toLeaderLineAnimationOptions = (options) => {
  if (options) {
...
// src/consts.ts
var ARROW_IDENTIFIER_CLASS = "arrow-identifier";
var ARROW_IDENTIFIER_PRETTIFIED_CIRCLE_CLASS = "arrow-identifier-prettified-circle";
var ARROW_CONTAINER_CLASS = "arrow-container";
var ARROW_SIZE = 3;
var DIAGONAL = "diagonal";
var MARGIN = "margin";
var arrowTypes = [DIAGONAL, MARGIN];
var ARROW = "arrow";
var NOARROW = "no-arrow";
var arrowPlugTypes = [ARROW, NOARROW];
var DISC = "disc";
var MARGIN_ARROW_X_OFFSET = 20;
var MARGIN_ARROW = "arrow1";
var DIAGONAL_ARROW = "arrow2";

// src/utils.ts
function rangeWithinExcludedContext(from, to, state) {
...
function arrowSourceToArrowIdentifierData(arrowSource) {
...
function arrowIdentifierCollectionIsResolved(arrowIdentifierCollection) {
  if (!arrowIdentifierCollection.start)
    return false;
  return arrowIdentifierCollection.ends.length > 0;
}
function getUserDefinedColorsDict(arrowSettings) {
  const dict = {};
...
function colorToEffectiveColor(colorName, arrowSettings) {
...
function getStartEndArrowPlugs(arrowheadName, arrowStartPlug, arrowEndPlug) {
...
function fixMarginArrowTrackNo(track) {
  if (track === 10) {
    return 11;
  } else if (track >= 11) {
    return track + 10;
...
function makeArrowArc(line, radius) {
  function addArc(pathData, radius2) {
    const reL = /^L ?([\d.\-+]+) ([\d.\-+]+) ?/;
    let newPathData, curXY, curDir, newXY, newDir, sweepFlag, arcXY, arcStartXY;
    ...
    pathData = pathData.trim().replace(/,/g, " ").replace(/\s+/g, " ").replace(/^M ?([\d.\-+]+) ([\d.\-+]+) ?/, function(s, x, y) {
      curXY = { x: +x, y: +y };
      return "";
    });
    if (!curXY) {
    ...
    while (pathData) {
      ...
      if (curDir) {
        arcStartXY = offsetXY(curXY, curDir, radius2, true);
        arcXY = offsetXY(curXY, newDir, radius2, false);
        sweepFlag = curDir === "l" && newDir === "u" ? "1" : curDir === "l" && newDir === "d" ? "0" : curDir === "r" && newDir === "u" ? "0" : curDir === "r" && newDir === "d" ? "1" : curDir === "u" && newDir === "l" ? "0" : curDir === "u" && newDir === "r" ? "1" : curDir === "d" && newDir === "l" ? "1" : curDir === "d" && newDir === "r" ? "0" : null;
        if (!sweepFlag) {
          throw new Error("Invalid data");
        }
        newPathData += "L" + arcStartXY.x + " " + arcStartXY.y + "A " + radius2 + " " + radius2 + " 0 0 " + sweepFlag + " " + arcXY.x + " " + arcXY.y;
      }
      curXY = newXY;
      curDir = newDir;
    }
...
function posToOffscreenPosition(view, pos) {
...
function offsetPositionsEqual(a, b) {
  return a.offsetLeft === b.offsetLeft && a.offsetTop === b.offsetTop;
}
function arrowIdentifierDataEqual(a, b) {
  return a.identifier === b.identifier && a.color === b.color && a.opacity === b.opacity && a.track === b.track && a.type === b.type && a.arrowArrowhead === b.arrowArrowhead;
}
function arrowRecordsEqual(a, b) {
  return a.startEl === b.startEl && a.endEl === b.endEl && arrowIdentifierDataEqual(a.startArrowData, b.startArrowData) && arrowIdentifierDataEqual(a.endArrowData, b.endArrowData) && a.startOffscreen == b.startOffscreen && a.endOffscreen == b.endOffscreen && offsetPositionsEqual(a.startElPos, b.startElPos) && offsetPositionsEqual(a.endElPos, b.endElPos);
}
function iterateCM6(workspace, callback) {
  workspace.iterateAllLeaves((leaf) => {
    leaf?.view instanceof import_obsidian2.MarkdownView && leaf.view.editor?.cm instanceof import_view.EditorView && callback(leaf.view.editor.cm);
  });
}

// src/arrowsConfig.ts
var import_state2 = require("@codemirror/state");
var arrowsConfig = import_state2.Facet.define({
  combine: (input) => {
    if (input.length > 0) {
      return input[0];
    } else {
...
function getArrowConfigFromView(view) {
  return view.state.facet(arrowsConfig);
}
var arrowsConfigCompartment = new import_state2.Compartment();
function getArrowsConfigExtension(arrowPluginSettings) {
  return arrowsConfigCompartment.of(arrowsConfig.of(arrowPluginSettings));
}
function reconfigureArrowsConfig(arrowPluginSettings) {
  return arrowsConfigCompartment.reconfigure(arrowsConfig.of(arrowPluginSettings));
}

// src/arrowsManager.ts
var ArrowsManager = class {
  constructor(view, container) {
    this.view = view;
    this.container = container;
    this.arrows = /* @__PURE__ */ new Map();
  }
  drawArrows(arrowIdentifierCollections) {
    const view = this.view;
    const oldArrows = this.arrows;
    const newArrows = /* @__PURE__ */ new Map();
    for (const arrowIdentifierCollection of arrowIdentifierCollections) {
      ...
      for (const end of arrowIdentifierCollection.ends) {
        ...
        if (startOffscreen != 0 && startOffscreen == endOffscreen)
          continue;
  ...
  addRecordToMap(map, endEl, record) {
    if (map.has(endEl)) {
  ...
  removeRecordFromMap(map, endEl, recordIndex) {
    if (!map.has(endEl))
      return;
  ...
  lineAlreadyExists(startEl, endEl, startArrowData, endArrowData, startOffscreen, endOffscreen) {
  ...
  drawArrow(startEl, endEl, startArrowData, endArrowData, startOffscreen, endOffscreen) {
    let line;
    if (startArrowData.type == DIAGONAL) {
  ...
  drawDiagonalArrow(startEl, endEl, startArrowData, endArrowData) {
    if (startEl == endEl)
      return;
    const color = colorToEffectiveColor(startArrowData.color, getArrowConfigFromView(this.view));
    const plugs = getStartEndArrowPlugs(DIAGONAL_ARROW, startArrowData.arrowArrowhead, endArrowData.arrowArrowhead);
    const line = new LeaderLine2({
  ...
  drawMarginArrow(startEl, endEl, startArrowData, endArrowData, startOffscreen, endOffscreen) {
    const res = this.getMarginArrowStartEndAnchors(startEl, endEl, startOffscreen, endOffscreen);
    if (!res)
      return;
    const { startAnchor, endAnchor } = res;
    const color = colorToEffectiveColor(startArrowData.color, getArrowConfigFromView(this.view));
    const plugs = getStartEndArrowPlugs(MARGIN_ARROW, startArrowData.arrowArrowhead, endArrowData.arrowArrowhead);
  ...
  getArrowRecord(line, startEl, endEl, startArrowData, endArrowData, startOffscreen, endOffscreen) {
    const arrowRecord = {
      line,
      startEl,
      endEl,
  ...
  arrowIdentifierPosDataToDomElement(arrow) {
    const view = this.view;
    try {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        el = node.childNodes[offset - 1];
      }
      if (!el)
        return;
      if (el.hasClass(ARROW_IDENTIFIER_PRETTIFIED_CIRCLE_CLASS)) {
        return el;
      } else {
  ...
  getMarginArrowStartEndAnchors(start, end, startOffscreen, endOffscreen) {
    const s = start.closest(".cm-line");
    if (!(s instanceof HTMLElement))
      return;
    let sy;
    if (startOffscreen != 0) {
      sy = startOffscreen * 1e3;
    } else {
    ...
    return {
      startAnchor: LeaderLine2.PointAnchor(s, { x: -MARGIN_ARROW_X_OFFSET, y: sy }),
      endAnchor: LeaderLine2.PointAnchor(e, { x: -MARGIN_ARROW_X_OFFSET, y: ey })
    };
  }
  removeAllArrows(arrows) {
    if (!arrows)
      arrows = this.arrows;
...
// src/arrowsViewPlugin.ts
var arrowSourceRegex = /{([^{}]+)}/g;
var arrowIdentifierHighlighter = new MatchDecoratorAll({
  regexp: arrowSourceRegex,
  decoration: (match) => {
    const arrowSource = match[1];
    const arrowData = arrowSourceToArrowIdentifierData(arrowSource);
    const startOrEnd = "";
    const className = ARROW_IDENTIFIER_CLASS + " " + startOrEnd;
...
var refreshAllArrows = import_state3.StateEffect.define();
var ArrowsViewPlugin = class {
  constructor(view) {
  ...
  createContainer(view) {
    ...
    container.addClass(ARROW_CONTAINER_CLASS);
    view.scrollDOM.prepend(container);
    this.container = container;
  }
  initialiseArrows(view) {
  ...
  arrowIdentifierRangesToArrowIdentifierPosData(arrowIdentifierRanges, state) {
    const rangeCursor = arrowIdentifierRanges.iter();
    const arrowIdentifierPosData = [];
    while (rangeCursor.value != null) {
      ...
      arrowIdentifierPosData.push({ from, to, arrowData });
      rangeCursor.next();
    }
    return arrowIdentifierPosData;
  }
  collectArrowIdentifierPosData(arrowIdentifierPosData) {
    const result = {};
    for (const arrowIdentifierPos of arrowIdentifierPosData) {
      ...
      if (!(identifier in result)) {
        result[identifier] = { identifier, ends: [] };
      }
      if (isStart) {
  ...
  arrowIdentifierCollectionsToDecos(arrowIdentifierCollections, view) {
    const decos = [];
    for (const arrowIdentifierCollection of arrowIdentifierCollections) {
      ...
      for (const [index, arrowIdentifier] of allArrowIdentifiers.entries()) {
        ...
        if (isResolved && !shouldUnravel) {
          ...
          deco = import_view2.Decoration.replace({
          ...
          deco = import_view2.Decoration.mark({
            tagName: "span",
            class: ARROW_IDENTIFIER_CLASS,
            arrowIdentifierPosData: arrowIdentifier
          }).range(arrowIdentifier.from, arrowIdentifier.to);
        }
        decos.push(deco);
...
var arrowsViewPlugin = import_view2.ViewPlugin.fromClass(ArrowsViewPlugin, {
  ...
  eventHandlers: {
    mousedown: function(event, view) {
      ...
      if (!(target instanceof HTMLElement))
        return;
      if (!target.hasClass(ARROW_IDENTIFIER_PRETTIFIED_CIRCLE_CLASS))
        return;
      const decos = this.decorations;
      const rangeCursor = decos.iter();
      const pos = view.posAtDOM(target);
      while (rangeCursor.value != null) {
        if (rangeCursor.from === pos)
          break;
        rangeCursor.next();
      }
      ...
      view.dispatch({
        effects: import_view2.EditorView.scrollIntoView(scrollTo, { y: "center" })
      });
      event.preventDefault();
    }
  }
});
var PrettifiedCircle = class extends import_view2.WidgetType {
  constructor(color, arrowIdentifierData) {
    super();
    this.color = color;
  ...
  toDOM() {
    ...
    span.className = ARROW_IDENTIFIER_PRETTIFIED_CIRCLE_CLASS;
    span.textContent = "\u25CF";
    return span;
  }
  ignoreEvent() {
...
// src/main.ts
var ArrowsPlugin = class extends import_obsidian3.Plugin {
  async onload() {
